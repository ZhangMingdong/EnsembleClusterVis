...
void MyChartWidget::wheelEvent(QWheelEvent * event)
{
	if (m_nMouseState == 1) {
		if (event->delta() > 0)
		{
			_dbBaseZ += 10;
			if (_dbBaseZ > 80)_dbBaseZ = 80;
		}
		else
		{
			_dbBaseZ -= 10;
			if (_dbBaseZ < 0)_dbBaseZ = 0;
		}

	}
	else if (m_nMouseState == 2) {
		if (event->delta() > 0)
		{
			_dbStepZ += 0.1;
			if (_dbStepZ > 2) _dbStepZ = 2;
		}
		else
		{
			_dbStepZ -= 0.1;
			if (_dbStepZ < 0) _dbStepZ = 0;
		}
